...
class BabyPoseDataset(JointsDataset):
    '''
    "keypoints": {
        1 : "right_hand",
        2 : "right_elbow",
    ...
    def _do_python_keypoint_eval(self, res_file, res_folder):
        # debug: res_file
        print("Evaluating results from:", res_file)
        coco_dt = self.coco.loadRes(res_file)
        coco_eval = COCOeval(self.coco, coco_dt, 'keypoints')
        coco_eval.params.useSegm = None
        #sigma array changed
        coco_eval.params.kpt_oks_sigmas = np.array([.62, .72, .79, .79, .72, .62, 0.89, .87, 1.07, 1.07, .87, .89])/10.0  
        # TODO: metto tutto a 1 in pycocotools > cocoeval.py > COCOeval > computeOks r. 206 perché coco ha scelto che devono essere 17 e non modificabili
        # sigma indicano le varianze nelle annotazioni dei keypoint su tutti gli annotatori, ma noi abbiamo solo un annotatore per frame quindi 1
        coco_eval.evaluate()
